      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className={`flex items-center justify-between h-20 transition-all duration-300 ${isScrolled ? 'h-16' : 'h-20'}`}>
          <div className="flex-shrink-0">
            <button onClick={() => handleNavClick('home')} aria-label={`${settings.siteName} Home`} className="flex flex-col items-start">
              <span className={`text-xl md:text-2xl font-bold transition-all duration-300 ${shouldShowSolid ? 'text-gradient-primary' : 'text-white drop-shadow-lg'}`}>
                {settings.siteName}
              </span>
              <span className={`text-[10px] md:text-xs font-medium transition-all duration-300 ${shouldShowSolid ? 'text-brand-accent' : 'text-white/90'}`}>
